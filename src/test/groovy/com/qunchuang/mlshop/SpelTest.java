package com.qunchuang.mlshop;

import com.qunchuang.mlshop.graphql.QueryFilter;
import com.qunchuang.mlshop.model.Administ;
import com.qunchuang.mlshop.model.user.User;
import org.junit.Test;
import org.springframework.expression.EvaluationContext;
import org.springframework.expression.spel.support.StandardEvaluationContext;

import java.util.ArrayList;
import java.util.Collection;
import java.util.HashMap;
import java.util.Map;

import static com.validator.bos.core.RuleParser.parser;

/**
 * @author Curtain
 * @date 2018/10/29 17:57
 */
public class SpelTest {

    @Test
    public void test() {


        //1.首先准备测试数据
        Collection<Integer> collection = new ArrayList<Integer>();
        collection.add(4);
        collection.add(5);
        Map<String, Object> map = new HashMap<String, Object>();
        Administ administ = new Administ();
        administ.setUsername("aaaaaaaaa");
        User user = new User();
        user.setUsername("uuuuuuuuu");
        map.put("admin", administ);
        map.put("user", user);

        //2.集合或数组测试
//        EvaluationContext context1 = new StandardEvaluationContext();
//        context1.setVariable("collection", collection);
//        Collection<Integer> result1 =
//                parser.parseExpression("#collection.?[#this>4]").getValue(context1, Collection.class);
//        Assert.assertEquals(1, result1.size());
//        Assert.assertEquals(new Integer(5), result1.iterator().next());

        //3.字典测试
        EvaluationContext context2 = new StandardEvaluationContext();
        context2.setVariable("administ", administ);



        Administ value1 = parser.parseExpression("new com.qunchuang.mlshop.model.Administ()").getValue(context2, Administ.class);

        QueryFilter value2 = parser.parseExpression("new com.qunchuang.mlshop.graphql.QueryFilter('id',EQUEAL,'XX',AND,null)").getValue(QueryFilter.class);

        String value = parser.parseExpression("#administ.username").getValue(context2, String.class);


    }
}
